...
const _0 = código => {
  ...
  return [statusSeq, [importações, carregamentos, outer_scope_param => {
    ...
    // atribuições_val is an array of items like:
    // [[nome_val, maybeSpace1, "=", maybeSpace2, valorAtribuição_fn], maybeOuterSpace]
    // So, the actual declaration is the first element of each item.
    for (const [decl_val_arr] of atribuições_val) {
      const [nome_val, , , , /* valorAtribuição_fn */] = decl_val_arr;
      blockScope[nome_val] = undefined; // Placeholder
    }

    // Second pass: Iterate over all constant declarations again.
    // For each declaration `nome_val = valorAtribuição_fn`, call `valorAtribuição_fn(blockScope)`.
    // `valorAtribuição_fn` is the function generated by the parser for the RHS of the assignment.
    // It takes a scope and returns the evaluated value.
    // If `valorAtribuição_fn` defines a lambda, that lambda will lexically capture `blockScope`.
    // Since `blockScope` now contains `nome_val` (from the first pass), the lambda can recursively refer to itself.
    // The result of this evaluation is then assigned to `blockScope[nome_val]`, replacing the placeholder.
    for (const [decl_val_arr] of atribuições_val) {
      const [nome_val, , , , valorAtribuição_fn] = decl_val_arr;
