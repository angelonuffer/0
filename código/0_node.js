...
// Helper function to parse a module on demand (async for lazy loading)
const parsear_módulo = async (endereço) => {
  ...
  // Load content (async - supports both local and remote)
...
// Helper function to evaluate a module and its dependencies lazily (async for lazy loading)
const avaliar_módulo = async (endereço) => {
  ...
  // Parse module if not yet parsed (lazy parsing - async)
  ...
  // Eagerly evaluate all imports (but loading is lazy - only happens when accessed)
  const escopo_importações = {};
  for (const [nome, dep_end] of importações) {
    escopo_importações[nome] = await avaliar_módulo(dep_end);
  }
  ...
  if (declarações) {
    for (const decl of declarações) {
      try {
        const valor = avaliar(decl.valor, escopo);
        // If the value is a promise (from lazy thunk or module literal), await it
        escopo[decl.nome] = valor instanceof Promise ? await valor : valor;
      } catch (erro) {
  ...
  let valor;
  try {
    const resultado = corpoAst ? avaliar(corpoAst, escopo) : undefined;
    // If the result is a promise (from lazy thunk or module literal), await it
    valor = resultado instanceof Promise ? await resultado : resultado;
  } catch (erro) {
    // Check if it's a semantic analyzer error
    if (erro.é_erro_semântico) {
      const erro_endereço = erro.módulo_endereço || endereço;
...
try {
  // Evaluate the main module (will trigger lazy loading and evaluation of dependencies)
