...
try {
  ...
  while (true) {
    ...
    const statements = módulo_bruto.valor.statements || [];
  ...
  while (true) {
    ...
    const { importações, statements, expressão: corpoAst } = módulo;
    ...
    if (statements) {
      for (const stmt of statements) {
        if (stmt.tipo === 'declaração') {
          escopo[stmt.nome] = undefined;
        }
      }
    }
    
    // Second pass: process statements in order (declarations and debug commands)
    if (statements) {
      for (const stmt of statements) {
        if (stmt.tipo === 'declaração') {
          try {
            escopo[stmt.nome] = avaliar(stmt.valor, escopo);
          } catch (erro) {
