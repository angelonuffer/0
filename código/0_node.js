...
// Helper function to load content (local or remote) - always async for consistency
const carregar_conteúdo = async (endereço) => {
  if (!endereço) {
    throw new Error('endereço is required');
  }
  if (cache[endereço]) {
  ...
  if (endereço.startsWith("https://")) {
    ...
    const conteúdo = fs.readFileSync(endereço, 'utf-8');
    cache[endereço] = conteúdo;
    return conteúdo;
  }
};
...
// Helper function to extract all module references (imports and address literals)
const extrair_todas_referências = (conteúdo) => {
  const endereços = new Set();
  
  // Extract imports
  const importações_resultado = importações(conteúdo);
  if (importações_resultado.sucesso) {
    for (const { endereço } of importações_resultado.valor) {
      endereços.add(endereço);
    }
  }
  
  // Extract address literals - they can appear in various contexts:
  // Match any of:
  ...
  return Array.from(endereços);
};

// Helper to pre-load all module content recursively (async, runs at startup)
const pré_carregar_conteúdo = async (endereço, visitados = new Set()) => {
  if (visitados.has(endereço)) {
    return;
  }
  visitados.add(endereço);
  
  // Load this module's content
  if (!conteúdos[endereço]) {
    conteúdos[endereço] = await carregar_conteúdo(endereço);
  }
  
  // Extract all module references
  const referências = extrair_todas_referências(conteúdos[endereço]);
  
  // Recursively pre-load all dependencies
  for (const ref of referências) {
    const endereço_resolvido = resolve_endereço(endereço, ref);
    await pré_carregar_conteúdo(endereço_resolvido, visitados);
  }
};

// Helper function to parse a module on demand (synchronous, lazy)
const parsear_módulo = (endereço) => {
  ...
  // Content must already be loaded by pré_carregar_conteúdo
  if (!conteúdos[endereço]) {
    throw new Error(`Module content not pre-loaded: ${endereço}`);
  }
...
// Helper function to evaluate a module and its dependencies lazily (synchronous, lazy)
const avaliar_módulo = (endereço) => {
  ...
  // Eagerly evaluate all imports (content already pre-loaded, evaluation is lazy)
  const escopo_importações = {};
  for (const [nome, dep_end] of importações) {
    escopo_importações[nome] = avaliar_módulo(dep_end);
  }
...
try {
  // Pre-load all module content (async, happens at startup)
  await pré_carregar_conteúdo(módulo_principal);
  
  // Evaluate the main module (synchronous, lazy parsing and evaluation)
